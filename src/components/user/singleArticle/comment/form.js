...
export default function CommentForm ({isShowForm, setIsShowForm}) {
    return (
        <div className={`${isShowForm ? 'block' : 'hidden'} space-y-3`}>
            <TextAreaElement className="text-sm" placeholder="متن خود را وارد کنید" />
            <div className="text-left">
                <Button onClick={() => setIsShowForm(false)} outline variant="secondary" size="md" className="ml-2">انصراف</Button>
