...
export default function ShowArticle ({article}) {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 md:rounded-xl space-y-5 shadow-md">
            ...
            <h1 className="text-gray-600 dark:text-gray-300 text-xl font-semibold px-4 md:px-5">{isLanguageFA() ? article.title : t('singleArticlePage.title')}</h1>
            <p className="text-gray-500 dark:text-gray-300 text-justify leading-[2.5rem] px-4 md:px-5">{isLanguageFA() ? article.body : t('singleArticlePage.body')}</p>
            <div className="flex flex-grow items-end justify-between text-gray-500 dark:text-gray-200 p-4 md:p-5">
                <div className={`flex items-center ${isLanguageFA() && 'space-x-reverse'} space-x-1`}>
                    ...
                    <span className="mt-1">{timestampToPersianDate(article.created_at, language)}</span>
                </div>
