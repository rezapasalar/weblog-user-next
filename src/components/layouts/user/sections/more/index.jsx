...
export default function More () {
    ...
    const changeIsShow = useCallback((status) => setIsShow(status))

    const moreRef = useRef(null)

    const {i18n: {language}} = useTranslation()

    const clickOutsideHandler = ({target}) => moreRef.current && !moreRef.current.contains(target) && setIsShow(false)
